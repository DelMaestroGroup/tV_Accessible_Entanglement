...
function spatial_entropy(basis::AbstractSzbasis, A, d::Vector{Float64}, MaxOccupation::Int,alpha::Float64)
    ...
    S2_sp = (1/(1-alpha))*log(sum(S_sp.^(2*alpha)))   #Actually S_alpha
    S1_sp=0
    for k=1:length(S_sp)
        if S_sp[k]^2>0
    ...
    # Operational.
    Ss_op = [S for (S, n) in zip(Ss_raw, norms)]
    ...
    S2s_op = [sum(S.^(2*alpha)) for S in Ss_op]
    S2s_op = S2s_op/sum(S2s_op)                   #Probability Distribution (P_alpha)
    norms.=norms.^(alpha)
